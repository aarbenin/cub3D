...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/09/12 07:52:43 by ogoman            #+#    #+#             */
/*   Updated: 2024/09/12 07:54:53 by ogoman           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * 
 * Checks if all the necessary textures (north, south, east, west) are
 * loaded and reports 
 ...
 * 
 * Checks if the floor and ceiling colors are set in the game structure
...
{
	handle_error(ERR_INV_COLOR, g, NULL, g->tex.floor == -1
		|| g->tex.ceiling == -1);
}
 ...
 * and existence.
 * 
 * Ensures that exactly one argument is provided (the map file) and
 * that the file has a 
 * '.cub' extension. Checks if the file can be opened and reports
 * errors if the file 
...
void	check_file(int ac, char **av)
{
	...
	handle_error(ERR_INV_AC, NULL, NULL, ac != 2);
